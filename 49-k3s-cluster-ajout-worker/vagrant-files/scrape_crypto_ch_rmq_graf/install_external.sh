...
DEBIAN_FRONTEND=noninteractive apt install -y clickhouse-client
...
CREATE TABLE IF NOT EXISTS helloworld.table1 ON CLUSTER '{cluster}' (
         ...
         ENGINE = ReplicatedMergeTree('/var/lib/clickhouse/data/tables/table1', '{node}')
         ORDER BY (uid)
...
ENGINE = ReplicatedMergeTree('/var/lib/clickhouse/data/tables/jsonlogs', '{node}')
ORDER BY tuple();
"
}
